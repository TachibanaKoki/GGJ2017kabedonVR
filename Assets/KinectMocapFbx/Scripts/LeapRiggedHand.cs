...
namespace Leap.Unity 
{
	...
	public class LeapRiggedHand : HandModel
	{
		...
		public override void UpdateHand() 
		{
			if (palm != null) 
			{
				...
				//          }
				//        }
				Quaternion palmRotation = GetRiggedPalmRotation(); // * Reorientation();
				palm.rotation = smoothFactor != 0f ? Quaternion.Slerp(palm.rotation, palmRotation, smoothFactor * Time.deltaTime) : palmRotation;
			}
		...
		public Quaternion GetRiggedPalmRotation() 
		{
			if (hand_ != null) 
			{
				...
				Quaternion handRotation = CalculateRotation(trs);
				//Debug.Log (Handedness + " hand rotation: " + handRotation.eulerAngles);

				return handRotation * Reorientation();
			}

			if (palm) 
			{
				//Debug.Log (Handedness + "old rotation: " + palm.rotation.eulerAngles);
				return palm.rotation;
			}

			//Debug.Log (Handedness + "zero rotation: " + Quaternion.identity.eulerAngles);
		...
		private void assignRiggedFingersByName()
		{
			...
			if (palm) 
			{
				foreach (Transform t in children) 
				{
					LeapRiggedFinger preExistingRiggedFinger;
					preExistingRiggedFinger = t.GetComponent<LeapRiggedFinger>();
					...
					if (!preExistingRiggedFinger) 
					{
						...
						{
							thumb = t;
							LeapRiggedFinger newRiggedFinger = thumb.gameObject.AddComponent<LeapRiggedFinger>();
						...
						{
							index = t;
							LeapRiggedFinger newRiggedFinger = index.gameObject.AddComponent<LeapRiggedFinger>();
						...
						{
							middle = t;
							LeapRiggedFinger newRiggedFinger = middle.gameObject.AddComponent<LeapRiggedFinger>();
						...
						{
							ring = t;
							LeapRiggedFinger newRiggedFinger = ring.gameObject.AddComponent<LeapRiggedFinger>();
						...
						{
							pinky = t;
							LeapRiggedFinger newRiggedFinger = pinky.gameObject.AddComponent<LeapRiggedFinger>();
		...
		{
			LeapRiggedFinger[] fingerModelList = GetComponentsInChildren<LeapRiggedFinger>();
			...
			for (int i = 0; i < 5; i++) 
			{
				...
				fingerModelList [i].smoothFactor = smoothFactor;
		...
		{
			LeapRiggedFinger[] riggedFingers = GetComponentsInChildren<LeapRiggedFinger>();

			foreach(LeapRiggedFinger finger in riggedFingers)
			{
		...
		{
			this.OnBegin += HandReset;
			this.OnFinish += HandFinish;
		}
